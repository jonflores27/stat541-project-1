...
plastics |> 
  ...
  summarise(sum(grand_total), mean(num_events), .groups = "drop") |> 
  mutate(event_impact = `sum(grand_total)`/`mean(num_events)`) |> 
  select(country, `mean(num_events)`, event_impact) |> 
  gt()
...
```{r}
plastics |> 
  mutate(v_effort = grand_total/volunteers,
         event_impact = grand_total/num_events)
